...
class OscillatorLayer(nn.Module):
    ...
    def forward(self, input_features):
        ...
        r = self.initial_r.unsqueeze(0).repeat(B, 1).unsqueeze(-1)
        phi = self.initial_phi.unsqueeze(0).repeat(B, 1).unsqueeze(-1)

        input_f = input_features.unsqueeze(-1)

        omega = self.omega.unsqueeze(0).unsqueeze(-1)
        omega_i = omega
        omega_j = omega.transpose(1, 2)

        rho = omega_i / (omega_j + 1e-8)

        C = self.C.unsqueeze(0)
        theta = self.theta.unsqueeze(0)

        mu_t = torch.tensor(self.mu, device=device).view(1,1,1)

        for _ in range(self.num_steps):

            phi_i = phi
            phi_j = phi.transpose(1, 2)
            r_j = r.transpose(1, 2)

            # --- Phase term ---
            phase_term = rho * phi_j - phi_i + theta
            ...
            coupling_r = self.k * torch.sum(
                C * r_power * torch.cos(phase_term),
                dim=-1,
                keepdim=True
            )

            r_safe = torch.clamp(r, self.power_min, self.power_max)

            coupling_phi = self.k * torch.sum(
                C * (r_power / r_safe) * torch.sin(phase_term),
                dim=-1,
                keepdim=True
            )

            dr_dt = (mu_t - r**2) * r + coupling_r + input_f
...
class ODEFuc(nn.Module):
    ...
    def forward(self, t, state):
        ...
        #omega_safe = np.where(omega == 0, 1e-8, omega)  # Replace 0 with a small value
        omega_ratio = omega_safe[:, None] / omega_safe[None, :]
        ...
        drdt = (self.mu - r**2) * r                + coupling_r                + e * torch.cos(phi)                + ecg_input * torch.cos(phi)
...
def train_heart_model(ecg_target_signal, device):
    ...
    sim_osc_input = torch.tensor(simulate_coupled_oscillators(T=15, dt=0.01), dtype=torch.float32).to(device)
...
def train_mlp_on_frozen_brain(
    ...
    with torch.no_grad():
        sim_input = torch.tensor(
            simulate_coupled_oscillators(T=15, dt=0.01),
            dtype=torch.float32
        ).to(device)
...
# This section computes descriptive metrics comparing the actual EEG target signal with the predicted brain response. We split the sequence into Train (10s) and Test (5s) sections to evaluate the model's accuracy.
