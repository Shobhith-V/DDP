...
class OscillatorODEFunc(nn.Module):
    ...
    def forward(self, t, state):
        ...
        input_f = self.input_f[t_idx].unsqueeze(-1)
        ...
        dphi_dt = omega.unsqueeze(-1) + coupling_phi
...
class OscillatorLayer(nn.Module):
    ...
    def forward(self, input_features):
        ...
        # input_features : (T, N_osc)
        T = input_features.shape[0]

        # store time dependent forcing
        self.ode_func.input_f = input_features
        self.ode_func.fs = self.fs
...
with torch.no_grad():
    r_test, phi_test, theta_test, omega_test, alpha_test, rcos_phi_test =         model_test.solve(
...
with torch.no_grad():
    ...
    brain_drive_train = trained_mlp_model(hidden_train)
