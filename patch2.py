...
for i, line in enumerate(lines):
    if line.startswith("class OscillatorLayer(nn.Module):"):
        osc_start = i
    elif line.startswith("class ECGToOscillatorMLP(nn.Module):"):
        ecg_start = i
    elif line.startswith("class ODEFuc(nn.Module):"):
        ode_start = i
    elif line.startswith("class TorchRevHopfNetwork:"):
        hopf_start = i

part1 = lines[:osc_start]
part2 = lines[ecg_start:ode_start]
part3 = lines[hopf_start:]

# Join arrays back
full_text = "".join(part1) + new_osc + "\n\n" + "".join(part2) + new_odefuc + "\n" + "".join(part3)

# Further text replacement patches on the full string
old_ecg_mlp = "        osc_hidden = self.osc_layer(pre)         # (B, N_VNS * 2)"
new_ecg_mlp = "        osc_hidden = self.osc_layer(pre) + torch.cat([pre, pre], dim=-1) # (B, N_VNS * 2)"
full_text = full_text.replace(old_ecg_mlp, new_ecg_mlp)
...
full_text = full_text.replace(old_feat, new_feat)
...
full_text = full_text.replace(old_opt, new_opt)
...
full_text = full_text.replace(old_clip, new_clip)
...
full_text = full_text.replace(old_fb1, new_fb1)
...
full_text = full_text.replace(old_fb2, new_fb2)
...
full_text = full_text.replace(old_pm, new_pm)

with open('/home/shobs/Desktop/DDP/working.py', 'w') as f:
    f.write(full_text)

print("Patch applied successfully, writing working.py")
