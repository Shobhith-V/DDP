...
try:
    ...
    Sw_all = (sc_matrix / max_val) * 0.2 if max_val > 0 else sc_matrix
...
def compute_power_terms(r, phi, omega, theta):
    omega_safe = torch.clamp(omega, 2 * np.pi * 0.5, 2 * np.pi * 20.0)
...
class OscillatorLayer(nn.Module):
    def __init__(self, N_osc=16, T=2.0, fs=100, device="cpu",
        ...
        self.register_buffer(
            "initial_phi",
            2*np.pi*torch.rand(N_osc, device=device)
        )
...
class ECGToOscillatorMLP(nn.Module):
    ...
    def __init__(self, ecg_dim=50, N_VNS=16, hidden_dim=64, output_dim=16, device="cuda"):
        ...
        self.res_proj = nn.Linear(N_VNS, 2 * N_VNS)  # FIX 7: learnable residual projection
    ...
    def forward(self, ecg_features):  # [batch, ecg_dim] or [ecg_dim]
        ...
        osc_hidden = self.osc_layer(pre) + self.res_proj(pre)  # (B, N_VNS * 2)
...
class ODEFuc(nn.Module):
    def __init__(self, mu, eta_theta, eta_omega, eta_alpha,
        ...
        # Fixed normalized readout — NOT trainable, avoids optimization shortcut in Stage-1.
        self.register_buffer(
            'readout',
    ...
    def forward(self, t, state):
        ...
        D = torch.tensor(
            self.D_function(t.detach().cpu().item()),
            device=state.device,
            dtype=state.dtype
        )

        brain_state = r * torch.cos(phi)
        P = torch.sum(self.readout * brain_state)
        e = D - P
        ...
        domegadt = -self.eta_omega * e * r*torch.sin(phi)
        dalphadt = self.eta_alpha * e * r * torch.cos(phi)
        _ = e  # keep gradient connection
...
def pre_train_brain_model(eeg_processed, Sw_all, target_idx, non_zero_indices_per_row, t, D_function, device):
    ...
    r0 = 0.1 * np.ones(N)
    phi0 = 2 * np.pi * np.random.rand(N)  # FIX 1: random phases to break symmetry lock
    theta_random = np.pi * (2 * np.random.rand(N, N) - 1)
    theta0 = theta_random - theta_random.T

    model = TorchRevHopfNetwork(
        mu=0.25, eta_omega=0.002, eta_alpha=0.01, eta_theta=0.01,  # FIX 9: timescale separation
    ...
    for epoch in range(100):
        ...
        with torch.no_grad():
            ...
            P_out = torch.sum(
                model.ode_func.readout.unsqueeze(0)
                * (r * torch.cos(phi)),
                dim=1
            )
...
def train_mlp_on_frozen_brain(
    ...
    for epoch in range(200):
        ...
        P_out = torch.sum(model.ode_func.readout.unsqueeze(0) * (r * torch.cos(phi)), dim=1)
        loss = criterion(P_out, D_true)
        ...
        torch.nn.utils.clip_grad_norm_(ecg_to_osc_mlp.parameters(), 0.3)
        optimizer.step()
        scheduler.step(loss.item())
...
target_signal = target_signal - np.mean(target_signal)  # FIX 10: zero-mean target
...
with torch.no_grad():
    raw_drive = trained_mlp_model(hidden_repr)
    drive = raw_drive
    drive = drive / (drive.std(dim=0, keepdim=True) + 1e-6)
    brain_drive_full = 0.05 * drive
...
with torch.no_grad():
    ...
    P_out_baseline = model.ode_func.readout(r_final * torch.cos(phi_final)).squeeze(-1).detach().cpu().numpy()
...
with torch.no_grad():
    P_predicted = model.ode_func.readout(r_final * torch.cos(phi_final)).squeeze(-1).detach().cpu().numpy()
